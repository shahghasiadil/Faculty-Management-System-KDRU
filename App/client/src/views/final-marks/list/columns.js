...
// ** Store & Actions
import { getFinalMark, deleteFinalMark, archiveFinalMark } from '../store/action'
    ...
    cell: row => (
      <UncontrolledDropdown>
        ...
        <DropdownMenu right>
          <DropdownItem
            className='w-100'
            onClick={() => store.dispatch(archiveFinalMark(row.id))}
          >
          ...
          <DropdownItem
            tag={Link}
            to={`/views/final-mark/edit/${row.id}`}
            className='w-100'
            onClick={() => store.dispatch(getFinalMark(row.id))}
          >
            <Archive size={14} className='mr-50' />
            <span className='align-middle'>Edit</span>
          </DropdownItem>
          <DropdownItem className='w-100' onClick={() => store.dispatch(deleteFinalMark(row.id))}>
