...
// ** Store & Actions
import { getAllData, getData, restoreChance } from '../store/action'
  ...
  export const  ErrorToast = ({id}) => (
    <Fragment>
      ...
      <div className='toastify-body alert-danger'>
        ...
        Record deleted  <strong>Oops</strong> <Button.Ripple color='flat-info' onClick = {() => { store.dispatch(restoreChance(id)) }}>Undo</Button.Ripple>
        </span>
      </div>
    </Fragment>
  )
...
const ChancesList = () => {
  ...
  const store = useSelector(state => state.chances)
  console.log(store)
  // ** States
